...
const talentShowcaseSchema = new mongoose.Schema(
  {
    ...
    rulesMessage: {
      type: String,
      default:
        "Please watch all performances before voting. Vote for your favorite act. Be respectful in chat.",
    },
    commercialDuration: {
      type: Number,
...
talentShowcaseSchema.virtual("isVotingOpen").get(function () {
  ...
  const votingEnd = this.votingEndTime || this.calculateVotingEndTime();
  return (
    now >= votingStart && now <= votingEnd && (this.status === "live" || this.status === "voting")
  );
});
