...
const validateShowcaseCreation = [
  ...
  body("votingStartTime").optional().isISO8601().withMessage("Invalid voting start time"),

  body("votingEndTime")
    .optional()
    .isISO8601()
    .withMessage("Invalid voting end time")
    .custom((value, { req }) => {
  ...
  body("prizeDetails.amount")
    .optional()
    .isFloat({ min: 0 })
    .withMessage("Prize amount must be positive"),
  ...
  body("commercialContent")
    ...
    .trim()
    .isLength({ max: 2000 })
    .withMessage("Commercial content cannot exceed 2000 characters"),

  body("commercials").optional().isArray().withMessage("Commercials must be an array"),

  body("commercials.*.title")
    .optional()
    .trim()
    .isLength({ min: 1, max: 100 })
    .withMessage("Commercial title must be 1-100 characters"),
  ...
  body("commercials.*.videoUrl")
    ...
    .trim()
    .notEmpty()
    .withMessage("Commercial video URL is required"),
...
const validateContestantRegistration = [
  body("showcaseId")
    .notEmpty()
    .withMessage("Showcase ID is required")
    .isMongoId()
    .withMessage("Invalid showcase ID"),

  body("performanceTitle")
    .trim()
    .notEmpty()
    .withMessage("Performance title is required")
  ...
  body("performanceDescription")
    .trim()
    .notEmpty()
    .withMessage("Performance description is required")
...
const validateVote = [
  ...
  body("contestantId")
    .notEmpty()
    .withMessage("Contestant ID is required")
    .isMongoId()
...
const validateShowcaseId = [param("id").isMongoId().withMessage("Invalid showcase ID"), validate];

const validateContestantId = [
  param("id").isMongoId().withMessage("Invalid contestant ID"),
...
const validateJudgeScore = [
  ...
  body("judgeName").trim().notEmpty().withMessage("Judge name is required"),
...
const validateCommercialDeletion = [
  ...
  param("commercialIndex")
    .notEmpty()
    .withMessage("Commercial index is required")
    .isInt({ min: 0 })
    .withMessage("Commercial index must be a non-negative integer"),
...
const validateTimeAdjustment = [
  ...
  body("additionalMinutes")
    .notEmpty()
    .withMessage("Additional minutes is required")
    .isFloat()
    .withMessage("Additional minutes must be a number")
    .custom((value) => {
